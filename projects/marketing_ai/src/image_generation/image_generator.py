...
class ImageGenerator(ABC):
    ...
    def __subclasscheck__(cls, subclass: Any) -> bool:
        return hasattr(subclass, "generate_image") and callable(subclass.generate_image)
...
class DallEImageGenerator(ImageGenerator):
    ...
    def _validate_prompt(self, image_prompt: str) -> None:
        ...
        if len(image_prompt) > self.MAX_PROMPT_LENGTH:
            raise ValueError(
                f"Prompt must be less than {self.MAX_PROMPT_LENGTH} characters"
            )
    ...
    def generate_image(self, image_prompt: str, width: int, height: int) -> str:
        ...
        response = openai.Image.create(
            prompt=image_prompt, model=self.model_version, n=1, size=size
        )
        parse_url = response["data"][0]["url"]

        logging.info(
            f"Generated image from prompt: {image_prompt}, size: {size}, url: {parse_url}"
        )
